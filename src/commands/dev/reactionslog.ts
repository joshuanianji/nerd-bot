...
export const reactionslog = async (client: Client, intr: ChatInputCommandInteraction): Promise<void> => {
    ...
    const prismaData = await client.prisma.reaction.findMany({
        orderBy: {
            createdAt: 'desc'
        },
        take: count,
    ...
    const reactionFields: APIEmbedField[] = await Promise.all(prismaData.map(async (data, i) => {
        ...
        const scoreA = await getScore(data.userId, client.prisma, data.createdAt);
        const scoreB = await getScore(data.message.authorId, client.prisma, data.createdAt);
        return {
